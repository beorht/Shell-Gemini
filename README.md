...
Для удаления ShellGemini из системы:
...
Скрипт удалит исполняемый файл и предложит удалить конфигурацию.
...
**Решение:**
1. Проверьте правильность ключа в `~/.config/shell-gemini/.shell-gemini`
...
1. **Никогда не коммитьте конфиг с ключом в git**
   ...
   - Конфиг в `~/.config/shell-gemini/` находится вне репозитория
...
2. **Проверьте права доступа к конфигу:**
   ...
   Скрипт `install.sh` автоматически устанавливает правильные права.
...
4. **Если ключ скомпрометирован:**
   ...
   - Создайте новый ключ
   - Обновите конфиг: `nano ~/.config/shell-gemini/.shell-gemini`
...
Чтобы сменить модель, отредактируйте строку 67 в `src/shell-gm.sh` (или `/usr/local/bin/shell-gm` после установки):
